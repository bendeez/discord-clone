...
async def test_create_friend_request(http_request,current_user,current_user_token,remote_user):
    ...
    response = await http_request(path="/friendrequest",method=RequestMethod.POST,
...
async def test_invalid_create_friend_request(http_request,current_user,current_user_token,remote_user,db):
    remote_user, _ = await remote_user()
...
async def test_get_friend_requests(http_request,current_user,current_user_token,remote_user,db):
    remote_user, _ = await remote_user()
    await create_friend_request(db=db, current_user=remote_user,
                                remote_user_username=current_user.username)
    response = await http_request(path="/friendrequests", method=RequestMethod.GET,
                                  token=current_user_token)
    assert response.status_code == 200
    data = response.json()
    assert data == [{"receiver":current_user.username,"receiverprofile":current_user.profile,
                       "sender":remote_user.username,"senderprofile":remote_user.profile}]

async def test_delete_friend_request(http_request,current_user,current_user_token,remote_user,db):
    remote_user, _ = await remote_user()
    await create_friend_request(db=db, current_user=remote_user,
                                remote_user_username=current_user.username)
    response = await http_request(path="/friendrequest", method=RequestMethod.DELETE,
                                  json={"username": remote_user.username},
                                  token=current_user_token)
