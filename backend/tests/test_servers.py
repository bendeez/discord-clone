...
async def test_create_server(http_request,current_user,current_user_token,websocket_connection):
        await websocket_connection(token=current_user_token) # to send the create server message (application does that)
...
async def test_get_servers(http_request,current_user,current_user_token,db):
    server = await create_new_server(server=ServerIn(name=f"{current_user.username} server"),
                                     current_user=current_user,
                                     db=db)
    response = await http_request(f"/servers", method=RequestMethod.GET, token=current_user_token)
    assert response.status_code == 200
    data = response.json()
    assert data == [{"id":server.id,"profile":server.profile,"owner":server.owner,"name":f"{server.owner} server"}]

async def test_get_server_information(http_request,current_user,current_user_token,db):
    server = await create_new_server(server=ServerIn(name=f"{current_user.username} server"),
                                     current_user=current_user,db=db)
    response = await http_request(f"/server/{server.id}", method=RequestMethod.GET, token=current_user_token)
    assert response.status_code == 200
    data = response.json()
    assert data == {"id": server.id, "profile": server.profile, "owner": server.owner,"name": f"{server.owner} server"}

async def test_get_server_information_forbidden(http_request,current_user_token):
    response = await http_request(f"/server/77", method=RequestMethod.GET, token=current_user_token)
...
async def test_get_server_users(http_request,current_user,current_user_token,db):
    server = await create_new_server(server=ServerIn(name=f"{current_user.username} server"),
                                     current_user=current_user,
                                     db=db)
    response = await http_request(f"/server/users/{server.id}", method=RequestMethod.GET, token=current_user_token)
    assert response.status_code == 200
    data = response.json()
    assert data == [{"username":current_user.username,"profile":current_user.profile,"status":current_user.status}]


async def test_join_server(http_request,current_user,current_user_token,remote_user,db,websocket_connection):
    """
    ...
    assert response.status_code == 201
    data = response.json()
    assert data == {"id":data["id"],"username":remote_user.username,"server_id":server.id}
    """
...
async def test_get_server_messages(http_request,current_user,current_user_token,db):
    server = await create_new_server(server=ServerIn(name=f"{current_user.username} server"),
                                     current_user=current_user,
                                     db=db)
    server_message = ServerWebsocketText(**{"server": server.id, "username": current_user.username,
                                            "profile": current_user.profile,
                                            "date": datetime.now(), "text": "hello"})
    await save_message(data=server_message)
    response = await http_request(f"/servermessages/{server.id}", method=RequestMethod.GET, token=current_user_token)
    assert response.status_code == 200
    data = response.json()
