...
async def test_create_dm(http_request,current_user,current_user_token,websocket_connection,remote_user):
        remote_user, remote_token = await remote_user()
        remote_ws, _ = await websocket_connection(token=remote_token)
        await websocket_connection(token=current_user_token)  # current_user websocket connection to send create dm message (application does that)
        ...
        data = response.json()
        dm_id = data["id"]
        assert data == {"id": dm_id, "sender": current_user.username, "receiver": remote_user.username}
        """
            first 2 are status update notifications for each
            user
        """
        await remote_ws.recv()
        await remote_ws.recv()
        data = await remote_ws.recv()
        assert data == NotificationNewDm(**{"sender": current_user.username,
                                            "profile": current_user.profile,
                                            "receiver": remote_user.username}).model_dump()

async def test_invalid_post_dm(http_request,current_user,current_user_token,db,remote_user):
    remote_user, _ = await remote_user()
    await create_new_dm(current_user=current_user,remote_user_username=remote_user.username,db=db)
    """
...
async def test_get_dms(http_request,current_user,current_user_token,remote_user,db):
    remote_user,_ = await remote_user()
    dm = await create_new_dm(current_user=current_user,remote_user_username=remote_user.username,db=db)
...
async def test_get_dm_user_by_id(http_request,current_user,current_user_token,remote_user,db):
    remote_user,_ = await remote_user()
    dm = await create_new_dm(current_user=current_user,remote_user_username=remote_user.username,db=db)
...
async def test_get_dm_messages(http_request,current_user,current_user_token,remote_user,db):
    remote_user, _ = await remote_user()
    dm = await create_new_dm(current_user=current_user,remote_user_username=remote_user.username,db=db)
    dm_message = DmWebsocketText(**{"dm":dm.id,"otheruser":remote_user.username,"username":current_user.username,"profile":current_user.profile,
