...
async def current_user(db):
    user = await create_new_user(db=db,email="current_user@gmail.com",username="current_user",password="1234")
...
async def remote_user(db):
    ...
    async def _remote_user():
        ...
        remote_user = await create_new_user(email=f"{remote_username}@gmail.com",
