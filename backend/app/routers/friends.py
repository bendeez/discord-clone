...
@router.post("/friend")
async def accept_friend_request(friend_request: FriendRequestIn, current_user: Users = Depends(get_current_user),
    ...
    await create_friend(db=db, current_user_username=current_user.username,
                        remote_user_username=friend_request.username)
