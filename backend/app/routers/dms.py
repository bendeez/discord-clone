...
from app.crud.dms import check_already_created_dm,create_new_dm,get_all_dms,check_user_in_dm,get_all_dm_information,get_all_dm_messages
from typing import List

router = APIRouter()

@router.post("/dm")
async def create_dm(friend_request:FriendRequestIn, current_user: Users = Depends(get_current_user), db:AsyncSession = Depends(get_db)):
    already_dm = await check_already_created_dm(db=db,current_user_username=current_user.username,remote_user_username=friend_request.username)
    if already_dm:
        raise HTTPException(status_code=status.HTTP_409_CONFLICT, detail="There is already a dm")
    dm = await create_new_dm(db=db,current_user_username=current_user.username,remote_user_username=friend_request.username)
...
async def get_dm_information(dm_id:int, current_user: Users = Depends(get_current_user), db:AsyncSession = Depends(get_db)):
...
async def get_dm_messages(dm_id:int, current_user: Users = Depends(get_current_user), db:AsyncSession = Depends(get_db)):
    in_dm = await check_user_in_dm(db=db, current_user_username=current_user.username, dm_id=dm_id)
    if not in_dm:
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="You are not a part of this dm")
