...
async def create_user(user: UserCreate, db: AsyncSession = Depends(get_db)):
    ...
    user = await create_new_user(db=db, username=user.username, email=user.email, password=user.password)
    return user
...
async def update_profile_picture(file: UploadFile, current_user: Users = Depends(get_current_user),
                                 db: AsyncSession = Depends(get_db)):
    profile_picture = await update_current_profile_picture(db=db, current_user=current_user, file=file)
