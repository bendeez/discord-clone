...
class Server(BaseMixin):
    ...
    server_users: Mapped[list["Server_User"]] = relationship(
        back_populates="server", cascade="all", passive_deletes=True
    )
    server_messages: Mapped[list["Server_Messages"]] = relationship(
        back_populates="parent_server", cascade="all", passive_deletes=True
    )
