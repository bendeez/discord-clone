...
async def check_already_friends(db: AsyncSession, current_user: Users, remote_user_username: str):
    friend = await get_friend(db=db, current_user=current_user,
...
async def delete_current_friend(db: AsyncSession, friend: Friends):
    await db.delete(friend)
    await db.commit()
async def create_friend(db: AsyncSession, current_user: Users, remote_user_username: str,current_friend_request: FriendRequests):
