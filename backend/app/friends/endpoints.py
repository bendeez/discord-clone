...
@friend_router.post(
    "/friend", status_code=status.HTTP_201_CREATED, response_model=FriendCreated
)
async def accept_friend_request(
    ...
    current_friend_request = await get_friend_request(
        db=db, current_user=current_user, remote_user_username=friend.username
    )
    if current_friend_request is None:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Friend request already deleted",
        )
    if current_friend_request.sender == current_user.username:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="Sender of friend request cannot accept it",
        )
    friend = await create_friend(
        db=db,
        current_user=current_user,
        remote_user_username=friend.username,
        current_friend_request=current_friend_request,
    )
    return friend
...
@friend_router.delete("/friend", status_code=status.HTTP_204_NO_CONTENT)
async def delete_friend(
    friend: FriendIn,
    current_user: Users = Depends(get_current_user),
    db: AsyncSession = Depends(get_db),
):
    current_friend = await get_friend(
        db=db, current_user=current_user, remote_user_username=friend.username
    )
