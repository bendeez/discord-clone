...
class ServerService(BaseService):
    ...
    async def create_new_server(
        self, db: AsyncSession, server: ServerIn, current_user: Users
    ):
        if server.profile:
    ...
    async def get_all_servers(self, db: AsyncSession, current_user: Users):
        servers = await db.execute(
            select(Server.owner, Server.profile, Server.id, Server.name)
    ...
    async def check_user_in_server(
        self, db: AsyncSession, server_id: int, current_user: Users
    ):
        server = await db.execute(
            select(Server)
            .where(Server.id == server_id)
            .options(selectinload(Server.server_users).selectinload(Server_User.user))
        )
        server = server.scalars().first()
        if server is not None:
            if current_user in [
                server_user.user for server_user in server.server_users
            ]:
                return True
        return False
    ...
    async def get_all_server_users(self, db: AsyncSession, server_id: int):
        users = await db.execute(
            select(Users.username, Users.profile, Users.status)
            .join_from(Server_User, Server_User.user)
            .where(Server_User.server_id == server_id)
        )
        return users.all()

    async def add_user_to_server(self, server_id: int, current_user: Users):
        server_user = await self.transaction.create(
            model_instance=Server_User(
    ...
    async def save_server_message(self, data: dict):
        server_message = await Server_Messages.save_dm_message(data=data)
        server_message = await self.transaction.create(server_message)
    ...
    async def send_create_server_message(self, current_user: Users, new_server: Server):
        ...
        central_ws_interface.add_valid_server_or_dm(
            usernames=[new_server.owner], type="server_ids", id=new_server.id
        )
        create_server_message = ServerWebsocketAnnouncement(
            **{
                "server": new_server.id,
                "announcement": f"{new_server.owner} has created the server",
                "username": new_server.owner,
            }
        )
        await central_ws_interface.broadcast_from_route(
            sender_username=current_user.username, message=create_server_message
        )

    async def send_join_server_message(
        self, current_user: Users, server_user: Server_User
    ):
        """
        add the server id to the users' server_ids list so they can
        send messages in that server
        """
        join_server_message = ServerWebsocketAnnouncement(
            **{
                "server": server_user.server_id,
                "announcement": f"{current_user.username} has joined the server",
                "username": server_user.username,
            }
        )
        central_ws_interface.add_valid_server_or_dm(
            usernames=[server_user.username],
            type="server_ids",
            id=server_user.server_id,
        )
        await central_ws_interface.broadcast_from_route(
            sender_username=current_user.username, message=join_server_message
        )
