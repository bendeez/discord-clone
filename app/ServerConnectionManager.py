...
async def save_message(data: dict, db: AsyncSession):
    ...
    if chat == "dm":
        ...
        if type == "link":
            ...
            message = models.Dm_Messages(link=link, username=username, dm=dm, serverinviteid=int(server_invite_id),
                                         created_date=datetime.now())
            db.add(message)
    ...
    if chat == "server":
        ...
        if type == "announcement":
            ...
            server = data["server"]
            message = models.Server_Messages(announcement=announcement, username=username, server=int(server),
                                             created_date=datetime.now())
            db.add(message)
