...
from schemas import FriendRequestIn,FriendsOut
...
@router.post("/friend")
async def accept_friend_request(friend_request:FriendRequestIn, current_user: models.Users = Depends(oauth.get_current_user), db:AsyncSession = Depends(get_db)):
...
async def get_friends(current_user: models.Users = Depends(oauth.get_current_user), db:AsyncSession = Depends(get_db)):
    friends = await db.execute(select(models.Users.profile, models.Users.username, models.Users.status, models.Dms.id.label("dmid")).join(models.Friends, or_(and_(
